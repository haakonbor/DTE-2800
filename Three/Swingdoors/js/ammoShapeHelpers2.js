...
export function createCompoundWallAndDoorShape(scene) {

	// AMMO:
	let wallMass = 1000;
	let wallScale = {x: 16, y: 8, z: 1};

	let doorMass = 15;
	let doorScale = {x: 8, y: 1, z: 4};

	let wallLeftShape = new Ammo.btBoxShape( new Ammo.btVector3( wallScale.x*0.5, wallScale.y*0.5, wallScale.z*0.5) );
	let wallRightShape = new Ammo.btBoxShape( new Ammo.btVector3( wallScale.x*0.5, wallScale.y*0.5, wallScale.z*0.5) );
	let wallCompoundShape = new Ammo.btCompoundShape();

	let doorLeftShape = new Ammo.btCylinderShape( new Ammo.btVector3(doorScale.x*0.5, doorScale.y*0.5, doorScale.z*0.5) );
	let doorRightShape = new Ammo.btCylinderShape( new Ammo.btVector3(doorScale.x*0.5, doorScale.y*0.5, doorScale.z*0.5) );
	let doorLeftCompoundShape = new Ammo.btCompoundShape();

	let trans1 = new Ammo.btTransform();
	trans1.setIdentity();
	trans1.setOrigin(new Ammo.btVector3(-5, 0, 0));
	wallCompoundShape.addChildShape(trans1, wallLeftShape);

	let trans2 = new Ammo.btTransform();
	trans2.setIdentity();
	trans2.setOrigin(new Ammo.btVector3(5, 0, 0));
	wallCompoundShape.addChildShape(trans2, wallRightShape);

	let trans3 = new Ammo.btTransform();
	trans3.setIdentity();
	trans3.setOrigin(new Ammo.btVector3(-2, 2, 0));
	compoundShape.addChildShape(trans3, footShape);
