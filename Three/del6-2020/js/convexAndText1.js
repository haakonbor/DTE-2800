...
function addModels() {
    ...
    //3D text:
    add3DText("Hva er klokka?", 100, 100, -50);
}
...
function createExtrudeMesh() {
    ...
    let geometry = getShape2();
    let gExtrude = new THREE.ExtrudeGeometry(geometry, options);
    let mExtrude = new THREE.MeshPhongMaterial({ color: 0x0FE70E, side: THREE.DoubleSide });
    let meshExtrude = new THREE.Mesh(gExtrude, mExtrude);
    meshExtrude.position.set(-150, 0, 150);
    scene.add(meshExtrude);
}
function getShape1() {
    // create a basic shape
    let shape = new THREE.Shape();
    shape.ellipse (0, 0, 80, 15);
    return shape;
}
