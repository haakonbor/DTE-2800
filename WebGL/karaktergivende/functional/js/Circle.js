...
function drawCircle() {
    ...
    if (!bindShaderParameters(gl.circleShaderProgram))
        return;
    gl.useProgram(gl.circleShaderProgram);
    ...
    let a_Position = gl.getAttribLocation(gl.circleShaderProgram, 'a_Position');
    let stride = (3 + 4) * 4;
    gl.vertexAttribPointer(a_Position, 3, gl.FLOAT, false, stride, 0);
    gl.enableVertexAttribArray(a_Position);

    let a_Color = gl.getAttribLocation(gl.circleShaderProgram, 'a_Color');
